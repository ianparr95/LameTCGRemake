...
public class ParsePokemonCardsFile {
	 ...
	 */
	public static String[][] getNameAndLevel() throws FileNotFoundException {
	 ...
	 * @throws FileNotFoundException
	 */
	private static int getNumberOfPokemonLoadedFromFile() throws FileNotFoundException{
		int c = 0;
		Scanner fScanner = new Scanner(POKEMON);
		while(fScanner.hasNextLine()) {
			String s = fScanner.nextLine();
	 ...
	 * @throws FileNotFoundException
	 */
	public static PokemonCard getPokemonCard(String name, String level) throws FileNotFoundException{
		Scanner fScanner = new Scanner(POKEMON);
		while(fScanner.hasNextLine()) {
			String s = fScanner.nextLine();
			if (getTag(s).equals("name")){
				...
				if (n.equals(name)) {
					...
					if (getTag(s).equals("level")){
						...
						if (l.equals(level)) {
							...
							// may be a duplicate name but different level - search on
							continue;
						}
					} else {
